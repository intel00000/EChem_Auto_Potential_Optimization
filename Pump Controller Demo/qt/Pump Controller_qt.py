...
class PicoController(QtWidgets.QMainWindow):
    ...
    def connect_to_pico(self):
        ...
        if selected_port:
            ...
            try:
                ...
                QMessageBox.information(self, "Connection Status", f"Successfully connected to {selected_port}")
                ...
                QMessageBox.critical(self, "Connection Status", f"Failed to connect to {selected_port}")
    ...
    def disconnect_pico(self, show_message=True):
        if self.serial_port:
            ...
            if show_message:
                QMessageBox.information(self, "Disconnection Status", "Successfully disconnected from Pico")
    ...
    def stop_procedure(self):
        if self.scheduled_task:
            self.killTimer(self.scheduled_task)
            self.scheduled_task = None
        self.start_time = -1
        self.total_procedure_time = -1
        self.current_index = -1
        ...
        QMessageBox.information(self, "Procedure Stopped", "The procedure has been stopped.")
    ...
    def send_command(self):
        try:
            ...
            QMessageBox.critical(self, "Connection Error", "Connection to Pico lost. Please reconnect to continue.")
    ...
    def load_recipe(self):
        file_path, _ = QFileDialog.getOpenFileName(self, "Open Recipe File", "", "CSV Files (*.csv);;Excel Files (*.xlsx *.xls)")
        if file_path:
            try:
                self.clear_recipe()
                ...
                self.ui.recipeTable.setHorizontalHeaderLabels(list(self.recipe_df.columns) + ["Progress Bar", "Remaining Time"])

                for index, row in self.recipe_df.iterrows():
                    for col_index, value in enumerate(row):
                        self.ui.recipeTable.setItem(index, col_index, QtWidgets.QTableWidgetItem(str(value)))
                ...
                QMessageBox.information(self, "File Load", f"Recipe file loaded successfully: {file_path}")

            except Exception as e:
                QMessageBox.critical(self, "File Load Error", f"Failed to load recipe file {file_path}: {e}")
    ...
    def start_procedure(self):
        ...
        for i, row in self.recipe_df.iterrows():
            for col_index, value in enumerate(row):
                self.ui.recipeTable.setItem(i, col_index, QtWidgets.QTableWidgetItem(str(value)))
    ...
    def execute_procedure(self, index=0):
        ...
        if index >= len(self.recipe_df):
            ...
            QMessageBox.information(self, "Procedure Complete", "The procedure has been completed.")
            self.ui.stopButton.setEnabled(False)
            self.ui.pauseButton.setEnabled(False)
            self.ui.continueButton.setEnabled(False)
            return
        ...
        if elapsed_time < target_time:
            self.scheduled_task = self.startTimer(intended_sleep_time)
            return
    ...
    def execute_actions(self, index, pump_actions, valve_actions):
        ...
        self.update_status()
        self.scheduled_task = self.startTimer(100)

    def update_progress(self):
        if self.total_procedure_time == -1 or self.recipe_df is None or self.recipe_df.empty or self.pause_timepoint != -1:
            return
        elapsed_time = time.time() - self.start_time - self.pause_duration
        ...
        for i, row in self.recipe_df.iterrows():
            ...
            if elapsed_time < time_stamp:
                ...
                for col_index, value in enumerate(row):
                    self.ui.recipeTable.setItem(i, col_index, QtWidgets.QTableWidgetItem(str(value)))
                self.ui.recipeTable.setItem(i, len(row), QtWidgets.QTableWidgetItem(f"{row_progress}%"))
                self.ui.recipeTable.setItem(i, len(row) + 1, QtWidgets.QTableWidgetItem(f"{remaining_time_row}s"))
    ...
    def edit_pump(self, pump_id):
        ...
        power_pin, ok = QtWidgets.QInputDialog.getInt(self, "Power Pin", "Enter power pin ID:", int(pump["power_pin"]))
        if not ok:
            return
        direction_pin, ok = QtWidgets.QInputDialog.getInt(self, "Direction Pin", "Enter direction pin ID:", int(pump["direction_pin"]))
        if not ok:
            return
        initial_power_pin_value, ok = QtWidgets.QInputDialog.getInt(self, "Initial Power Pin Value", "Enter initial power pin value (0/1):", int(pump["initial_power_pin_value"]), 0, 1)
        if not ok:
            return
        initial_direction_pin_value, ok = QtWidgets.QInputDialog.getInt(self, "Initial Direction Pin Value", "Enter initial direction pin value (0/1):", int(pump["initial_direction_pin_value"]), 0, 1)
        if not ok:
            return
        initial_power_status, ok = QtWidgets.QInputDialog.getItem(self, "Initial Power Status", "Enter initial power status (ON/OFF):", ["ON", "OFF"], 0, False)
        if not ok:
            return
        initial_direction_status, ok = QtWidgets.QInputDialog.getItem(self, "Initial Direction Status", "Enter initial direction status (CW/CCW):", ["CW", "CCW"], 0, False)
        if not ok:
            return
