...
def serial_display_loop():
    ...
    while True:
        ...
        if count >= NUM_SAMPLES:
            ...
            print(
                f"adc1_avg raw: {adc1_avg:.3f}, adc1_avg scaled: {ADC1_value_scaled:.3f}, adc2_avg raw: {adc2_avg:.3f}, adc0_avg raw: {adc0_avg:.3f}",
                end=", ",
            )
            print(
                f"temp_avg raw: {temp_avg:.3f}, RP2040 internal temperature in Celsius: {temp_celsius:.3f} C, in Fahrenheit: {temp_fahrenheit:.3f} F",
                end=", ",
            )
            print(
            ...
            print(
                f"Free heap: {gc.mem_free()} bytes, Allocated heap: {gc.mem_alloc()} bytes"
            )
