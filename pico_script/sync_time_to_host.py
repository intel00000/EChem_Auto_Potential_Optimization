...
print(f"Found {len(picoPorts)} Raspberry Pi Pico(s) on port:" + str([p.device for p in picoPorts]))
utcTime = str( int(time.time()) )

print("Host computer epoch time: "+utcTime)
print("Host computer local time: "+time.strftime('%Y-%m-%d %H:%M:%S', time.localtime())+'\r')
